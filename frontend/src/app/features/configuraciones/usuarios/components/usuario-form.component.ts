    ...
    <form [formGroup]="form" (ngSubmit)="onSubmit()">
      <app-modal [title]="user() ? 'Editar Usuario' : 'Nuevo Usuario'">
        ...
        <div class="form-group">
          ...
          <div class="input-with-icon">
            <input [type]="showConfirmPassword() ? 'text' : 'password'" formControlName="confirmPassword" [placeholder]="user() ? 'Dejar en blanco para no cambiar' : ''">
          ...
          @if (form.errors?.['passwordMismatch'] && form.get('confirmPassword')?.touched) {
            <span class="error-text" style="color: red; font-size: 0.85rem; margin-top: 0.25rem; display: block;">Las contraseñas no coinciden</span>
          }
        </div>
